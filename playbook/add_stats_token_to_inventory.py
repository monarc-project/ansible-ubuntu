    ...
    for fo_server in fo_servers:
        ...
        with open(yaml_file, 'r') as stream:
            ...
            for client in clients_list:
                # check if in the client's config url and key.
                # create a new client on StatsService side if the config vars are not set:
                # statsToken = secrets.token_urlsafe(64)
                # go to stats server (BO) and directory or maybe api call ?
                # poetry run flask client_create --name {client['name']} --token {statsToken}
                # we need to update the inventory with the 'apiKey' = {statsToken}
                # Should be fine then :)
